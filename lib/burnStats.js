...
import { getReadProvider, getAmmPriceInEth, getEthUsdPrice } from "./pricing";

const HOUR = 60 * 60 * 1000;
const DAY = 24 * HOUR;

// Fallback window if the RPC rejects an unbounded eth_getLogs call (common
// on public nodes) — most public testnet RPCs cap this well below this.
...
async function queryBurnLogs(contract, provider) {
  try {
    return await contract.queryFilter(contract.filters.BurnExecuted(), 0, "latest");
  } catch {
    try {
      const latest = await provider.getBlockNumber();
      const fromBlock = Math.max(0, latest - FALLBACK_LOOKBACK_BLOCKS);
      return await contract.queryFilter(contract.filters.BurnExecuted(), fromBlock, "latest");
    } catch {
      return [];
    }
  }
