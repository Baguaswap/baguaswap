...
// Live "age since launch" label for a Hot Launchpad card, e.g. "7s",
// "45s", "1m", "23m", "4h", "2d" — counts up (seconds -> minutes -> hours
// -> days) instead of a static "New" label. Returns null (never a guessed
// value) if the launch timestamp genuinely isn't known yet.
export function formatTokenAge(createdAtMs) {
  if (createdAtMs == null) return null;
  const diffSec = Math.max(0, Math.floor((Date.now() - createdAtMs) / 1000));
