...
// Giwa's own docs mark the endpoint above as "rate-limited and should not
// be used in production." This is a free, no-signup public RPC for the
// same chain (Pocket Network) used purely as an automatic failover so a
// throttled/slow response from the primary doesn't get misread as "this
// token doesn't exist" — see getReadProvider() in lib/pricing.js.
export const BACKUP_RPC_URL =
  process.env.NEXT_PUBLIC_BACKUP_RPC_URL || "https://giwa-sepolia-testnet.api.pocket.network";
...
// BaguaBuybackBurn — buys BAG with collected protocol fees and burns it.
// Only the read side is used by the frontend today (Home tab burn stats):
// the BurnExecuted event is the sole on-chain record of "how much has been
// burned", since the contract itself keeps no running total in storage.
export const BUYBACK_BURN_ABI = [
  "function router() external view returns (address)",
  "function bagToken() external view returns (address)",
  "function treasury() external view returns (address)",
  "event BurnExecuted(uint256 amount)",
  "event BuybackExecuted(uint256 ethIn, uint256 tokensOut)",
];
