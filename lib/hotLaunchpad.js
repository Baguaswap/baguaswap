...
import { getReadProvider, getBondingCurvePriceInEth, getAmmPriceInEth, getEthUsdPrice, queryLogsWithFallback, withRetry } from "./pricing";
  ...
  try {
    ...
    const tokens = await Promise.all(
      addresses.map(async (address) => {
        ...
        try {
          const state = await withRetry(() => curve.tokens(address));
