...
import { getReadProvider, getBondingCurvePriceInEth, getAmmPriceInEth, getEthUsdPrice } from "./pricing";
...
// call. A single fixed window (the old approach) assumes we already know
// this RPC's exact range cap — but public nodes vary wildly (some accept
// 300k+ blocks, many cap out far lower, around 2k-10k). This cascades
// through decreasing windows and uses the first one the RPC actually
// accepts, instead of silently coming back empty when a single guessed
// window is still too large.
const FALLBACK_LOOKBACK_WINDOWS = [300_000, 50_000, 10_000, 2_000];

const CACHE_TTL_MS = 30_000;
let cache = null; // { data, fetchedAt }
let inflight = null;

async function queryAllLogs(contract, filter, provider) {
  try {
  ...
  for (const window of FALLBACK_LOOKBACK_WINDOWS) {
    try {
...
async function loadHotLaunchpadTokens() {
  ...
  try {
    ...
    const [buyLogsRaw, sellLogsRaw, createdLogsRaw, ethUsd, tokenSellSupply] = await Promise.all([
      queryAllLogs(curve, curve.filters.TokensPurchased(), provider),
      queryAllLogs(curve, curve.filters.TokensSold(), provider),
      queryAllLogs(factory, factory.filters.TokenCreated(), provider),
      getEthUsdPrice(),
