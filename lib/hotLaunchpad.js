...
// Fallback window if the RPC rejects an unbounded eth_getLogs call (common
// on public nodes) — mirrors lib/burnStats.js.
const FALLBACK_LOOKBACK_BLOCKS = 300_000;
...
async function queryAllLogs(contract, filter, provider) {
  try {
    ...
    try {
      ...
      const fromBlock = Math.max(0, latest - FALLBACK_LOOKBACK_BLOCKS);
...
async function loadHotLaunchpadTokens() {
  ...
  try {
    ...
    const [buyLogsRaw, sellLogsRaw, ethUsd, tokenSellSupply] = await Promise.all([
